...
// The frame worker encodes a sealed frame while the producer may already be reconfiguring the
// next one (live aspect switches change both values), so capture them when the frame is sealed.
struct FrameMapping {
